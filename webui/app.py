...
Adds two new endpoints on top of the standard web UI:
  POST /backend/set    {backend: hybrid|olive}  — switch solver backend
...
def dashboard():
    ...
    return render_template(
        ...
        solver_backend=(
            status.result.get("solver_backend", "hybrid")
            if status.ok else "hybrid"),
        test_mode=(
...
def backend_set():
    ...
    if backend not in ("hybrid", "olive"):
        return "backend must be 'hybrid' or 'olive'", 400
...
_CONFIG_SECTIONS = [
    ...
    ("Plate Solving", [
        ("olive_db",         "Solver DB",          "Star database (.npz) used by both hybrid and olive backends."),
        ...
        ("match_threshold",  "Match threshold",    "Max false-positive probability for accepting a match (1e-5 = default)."),
        ("match_radius",     "Match radius",       "Max centroid-to-catalog distance as fraction of FOV."),
    ]),
    ("Boresight", [
