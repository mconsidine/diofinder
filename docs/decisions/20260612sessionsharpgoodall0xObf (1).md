...
- `scripts/diofinder-update`: `--ref BRANCH` support
...
**Decision:** Remove `gate_mode="matched_filter"` from `diofinder/bg_cache.py`
...
**Rationale:** Allows on-device A/B tuning (`diofinder-bg-setup set top_hat --radius 12`)
...
| File | Change |
|------|--------|
| `diofinder/bg_cache.py` | Removed `gate_mode="matched_filter"` from cached-path and per-frame-path `kw` dicts. |
| `diofinder/comms_proc.py` | Extended `solver_params_get/set` for `detect_bg_mode` and `detect_tophat_radius`. Added `solve_centroids` maint command. Imported `SOLVER_OP_SOLVE_CENTROIDS`. |
| `diofinder/worker_cmds.py` | Added `SOLVER_OP_SOLVE_CENTROIDS = "solve_centroids"` constant. |
| `diofinder/solver_proc.py` | Added `solve_centroids` branch in `_handle_solver_cmd`; receives `solver_t3`, `cfg`, `shared_cfg`. |
...
| `scripts/diofinder-bg-setup` | New CLI: show/set `detect_bg_mode` and `detect_tophat_radius` via maint socket. |
| `scripts/diofinder-bg-test` | New A/B harness: detection counts, timing, centroid agreement across bg modes on saved or live frames. Supports `--inject-gradient`. |
| `scripts/diofinder-update` | Added `--ref BRANCH` to fetch + checkout + fast-forward a branch instead of only release tags. |
...
| `etc/diofinder.conf.default` | Added `detect_bg_mode`, `detect_tophat_radius`, and all `bg_cache_*` keys with comments. |
...
1. **Update the device:**
   ```bash
   sudo diofinder-update --ref olive
   ```
...
3. **Set and persist if top_hat is beneficial:**
   ```bash
   diofinder-bg-setup set top_hat --radius 14 --persist
...
- **Future bg modes** (`column_percentile`, `block_percentile`, `uniform_mean`,
  ...
  sycamore >= 0.10.0/0.11.0. Do not enable in `solver_params_set` or `diofinder-bg-setup`
