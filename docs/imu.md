...
snapping back at each solve. The motion gate suppresses that, and since
v0.11.16 it is a **rotation-RATE** gate (`imu_rate_gate_dps`, default
0.1°/s, measured over a ≥0.8 s baseline of poll-time samples): parked gyro
drift is ~0.01°/s while even a slow manual pan is >0.1°/s, so slow pans stay
smooth — the case the earlier displacement-since-solve gate froze (each solve
re-anchored the reference before sub-gate motion could accumulate). Once
engaged, the prediction **holds until the next solve re-anchors** after motion
stops, so stopping never snaps the crosshair back by the slew distance.
...
flowchart TD
    ...
    D --> G{"rotation RATE < imu_rate_gate_dps ?<br/>(default 0.1°/s over ≥0.8 s)"}
    G -->|"yes · STATIONARY<br/>(only gyro drift) — unless a slew<br/>just ended with no solve yet (HOLD)"| SOLVED
...
Result: a **parked scope sits still on the solved position**; the IMU takes
over for any real motion — fast or slow — and hands back to solves only after
motion stops *and* a fresh solve re-anchors (seamless, no snap-back). The gate
is on raw IMU rotation rate (independent of the calibration transform) and is
tunable via `imu_rate_gate_dps` (config + `shared_cfg`).
...
| `imu_rate_gate_dps` | comms (seed from cfg) | comms pointing | stationary rate threshold; below it (and once re-anchored) `:GR`/`:GD` report the solve (default 0.1°/s) |
